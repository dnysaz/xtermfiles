...
SSH freeze fix applied:
    SearchModal.on_input_changed now uses run_worker for the rglob scan
    so searching on a remote/slow filesystem doesn't block the UI.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
#  SearchModal
#  FIX: rglob scan now runs in a worker thread so it doesn't block the UI
#       on slow local filesystems or remote SSH paths.
# ─────────────────────────────────────────────────────────────────────────────
class SearchModal(ModalScreen[Optional[Path]]):
    def __init__(self, root):
        ...
        self._root         = root
        self._results: list = []
        self._search_gen   = 0   # counter to cancel stale searches
    ...
    def on_input_changed(self, e: Input.Changed):
        ...
        # Bump generation counter so any in-flight worker knows to discard results
        self._search_gen += 1
            ...
            try:
                for p in self._root.rglob("*"):
                    if gen != self._search_gen:
                        return   # newer search started, abandon this one
    ...
    def _finish_search(self, results: list):
...
# ─────────────────────────────────────────────────────────────────────────────
#  TextEditorModal — full-screen built-in editor
# ─────────────────────────────────────────────────────────────────────────────
class TextEditorModal(ModalScreen[bool]):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(classes="editor-box"):
            ...
            yield TextArea(content, language=use_lang, id="editor-area")
...
class SettingsModal(ModalScreen[bool]):
        ...
        with Vertical(id="settings-box"):
            ...
            with ScrollableContainer(id="settings-scroll"):
                ...
                for key, label, desc in self.BOOL_SETTINGS:
                    ...
                    yield Static(
                        f"[bold #d4d4d4]{label}[/bold #d4d4d4] [dim]{desc}[/dim]",
                        classes="stg-label",
                    )
                    yield Button(
                        " ON " if val else " OFF",
                ...
                # View Mode Toggle
                vm = self._s.get("view_mode")
                if not isinstance(vm, str): vm = "list"
                yield Static("[bold #d4d4d4]View Mode[/bold #d4d4d4] [dim]Toggle List or Grid layout[/dim]", classes="stg-label")
                yield Button(vm.upper(), id="tog-view_mode", classes="stg-btn-on")
    ...
    def on_button_pressed(self, e: Button.Pressed):
        bid = e.button.id or ""
        if bid == "tog-view_mode":
            curr = str(self._s.get("view_mode") or "list")
            new_mode = "grid" if curr == "list" else "list"
            self._s.set("view_mode", new_mode)
            e.button.label = new_mode.upper()
            return
...
class SSHLoginModal(ModalScreen[tuple[str, str]]):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="ssh-box", classes="dialog-box"):
            yield Label(f"SSH Login: {self.host}:{self.port}", id="ssh-title",
...
class LoadingModal(ModalScreen[None]):
    ...
    def on_button_pressed(self, event: Button.Pressed):
        if event.button.id == "loading-cancel-btn":
            self.dismiss(None)
# ─────────────────────────────────────────────────────────────────────────────
#  ContextMenuModal — triggered by Shift+Enter
# ─────────────────────────────────────────────────────────────────────────────
class ContextMenuModal(ModalScreen[Optional[str]]):
    def __init__(self, path: Path, has_paste: bool = False):
        super().__init__()
        self._path = path
        self._actions = [
            ("refresh",   "🔄 Refresh",     "R"),
        ...
        if has_paste:
            self._actions.append(("paste", "📋 Paste", "Ctrl+V"))
            
        self._actions.extend([
            ("cut",       "✂️ Cut",          "Ctrl+X"),
            ("rename",    "✏️ Rename",       "F2"),
            ("properties","ℹ️ Properties",   "Ctrl+I"),
        ])

    def compose(self) -> ComposeResult:
        with ScrollableContainer(id="context-menu-box"):
            yield Label(f" {self._path.name}", id="context-menu-title")
            with ListView(id="context-menu-list"):
                for cmd, label, key in self._actions:
                    yield ListItem(Label(Text.from_markup(f"{label:<18} [dim]{key}[/dim]")), id=f"ctx-{cmd}")
            with Horizontal(classes="modal-buttons"):
                yield Button("Close", id="btn-cancel", classes="btn-cancel")

    def on_mount(self):
        self.query_one("#context-menu-list", ListView).focus()
